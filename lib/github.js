 ...
 * Uses GitHub REST search API to search for commits with author-date filter.
...
export async function getAgentCommitCount(agentName, date) {
  return getCommitSearchCount(`author:${agentName} author-date:${date}`, `commits for ${agentName}`);
}

/**
 * Fetches the count of merged pull requests labeled as Codex on a given date.
 * Uses the public GitHub PR label signal.
 * @param {string} date - Date in YYYY-MM-DD format
 * @returns Number of merged Codex-labeled PRs on that date
 */
export async function getCodexPRLabelCount(date) {
  return getIssueSearchCount(`is:pr is:merged label:codex merged:${date}`, "Codex labeled PRs");
}

async function getCommitSearchCount(query, context) {
  if (!process.env.GITHUB_TOKEN) {
    throw new Error("GITHUB_TOKEN must be set");
  }
...
async function getIssueSearchCount(query, context) {
  if (!process.env.GITHUB_TOKEN) {
    throw new Error("GITHUB_TOKEN must be set");
  }
