...
/**
 * Fetches the count of commits attributed to Codex on a given date.
 * Uses the "Co-authored-by: Codex" trailer that GitHub Codex writes to commits.
 * @param {string} date - Date in YYYY-MM-DD format
 * @returns Number of commits co-authored by Codex on that date
 */
export async function getCodexCommitCount(date) {
  return getCommitSearchCount(`"Co-authored-by: Codex" author-date:${date}`, "Codex co-authored commits");
}
