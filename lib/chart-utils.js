 ...
 * @param {Array} codexData - Array of data points for Codex
 * @returns {Array} - Array of data points with {date, copilotCount, claudeCount, cursorCount, codexCount}
 */
export function mergeAgentData(copilotData, claudeData, cursorData = [], codexData = []) {
  ...
  codexData.forEach(item => {
    if (dateMap.has(item.date)) {
      dateMap.get(item.date).codexCount = item.count;
    } else {
      dateMap.set(item.date, {
        ...createEmptyDate(item.date),
        codexCount: item.count,
      });
    }
  });
