...
export async function fetchClaudePRData(options = {}) {
  try {
    const { limit = 365 } = options;
    const limitValue = Math.min(Math.max(limit, 1), 1000); // Cap at 1000 records

    // Get recent data points from Redis sorted set (most recent first)
    const data = await redis.zrange(CLAUDE_PR_KEY, -limitValue, -1, {
      withScores: false,
    });
...
export async function fetchCursorPRData(options = {}) {
  try {
    const { limit = 365 } = options;
    const limitValue = Math.min(Math.max(limit, 1), 1000); // Cap at 1000 records

    // Get recent data points from Redis sorted set (most recent first)
    const data = await redis.zrange(CURSOR_PR_KEY, -limitValue, -1, {
      withScores: false,
    });
